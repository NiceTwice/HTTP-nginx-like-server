...
namespace apouche {
    ...
    */
    ...
    public:
        ...
        */
        template< class ...Args>
        void callAllEvent(const std::function<void (void *, bool)> &fct, void *ptr, Args&&... args){
            for (auto &&it : _event) {
                fct(ptr, it.getFunction()(args...));
            }
         //   _event.clear();
        }

        template< class ...Args>
        bool callPollEvent(ReturnState &state, Args&&... args) {
            if (this->_event.size() > 0) {
                if (this->_ite == this->_event.end()) {
                  this->_ite = this->_event.begin();
                  return (false);
                }
                state = ((*(this->_ite)).getFunction())(args...);
                this->_ite++;
                if (state == apouche::FATAL_ERROR)
                  this->_ite = this->_event.begin();
                return (true);
            }
          return (false);
